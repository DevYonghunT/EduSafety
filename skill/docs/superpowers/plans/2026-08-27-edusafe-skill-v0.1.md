...
- **되돌리기 실패를 삼키지 않는다.** 되돌리지 못한 파일 목록과 보관 위치를 오류에 담아, 최상위가 섞였을 때 교사가 무엇을 손으로 되돌려야 하는지 알 수 있게 한다. 되돌리기도 주입 가능한 `rename` 을 쓰므로 이 경로까지 테스트가 지난다.
- **history 정리 대상은 스탬프 형식 디렉터리뿐이다.** `<YYYYMMDD>-<HHMMSS>-<16진수 4자리>` 형식에 맞지 않는 폴더는 건드리지 않는다 — 교사가 `history/` 아래에 따로 보관한 자료까지 지우면 그것은 파일 손실이다(리뷰에서 실측).

**지문은 렌더러가 계산한다.** AI 가 `self_reported_skill_digest` 에 임의 문자열을 적어도 통과하던 것을 막는다 — CLI 가 `skillDigest()` 로 지금 도는 스킬 폴더의 해시를 구해 보고서 JSON 에 채워 넣고, 그 상태로 검증·렌더한다. 계약의 `sha256:…` 제약은 sha256 뒤에 64자리 16진수가 오는지로 검사한다.
...
> `[REQ-5.1]` 0~6단계는 사람의 응답 없이 끝까지 진행한다. 질문을 던지고 기다리는 단계는 3단계(빌드 승인)와 7단계(확인 세션)뿐이다. 1단계는 스킬 폴더 경로를 얻지 못한 예외 경로에서만 대화형으로 1회 묻고, 비대화형에서는 묻지 않고 `degraded` 로 강등해 계속한다 — 어느 경우에도 사람의 응답을 기다려 멈추지 않는다.
...
> `[REQ-5.25]` SKILL.md 는 스크립트를 자신의 부모 폴더 기준 상대 경로(`scripts/scan.mjs`)로 지시한다. 경로를 얻을 수 없으면 전역 경로를 임의 탐색하지 않고 **대화형일 때만** 사용자에게 설치 경로를 1회 확인받으며, 비대화형이면 묻지 않고 바로 다음으로 넘어간다. 그래도 없으면 `coverage.runtime = "degraded"` 로 강등 실행한다.
...
2. 정규화(REQ-13.2): 경로 구분자 `/`, 줄바꿈 LF, 경로 오름차순 정렬. 줄바꿈 정규화는 **텍스트 확장자에만** 적용한다 — 바이너리를 utf8 로 왕복시키면 바이트가 손상된다(코덱스 리뷰 실측: PNG 12바이트 → 19바이트).
...
6. `dist/edusafe-v<버전>.zip` · `.sha256` · `manifest.json` 을 쓴다. `outDir` 는 통째로 지우므로, 그것이 스킬 폴더이거나 그 조상이면 **지우기 전에 거부한다** — 실측으로 `edusafe/` 가 산출물 3개로 대체되는 것을 확인했다.
...
9. CRC32 를 **zlib 의 독립 구현**으로 대조한다. 우리 `crc32()` 를 불러 비교하면 라이터가 틀릴 때 테스트도 같이 틀려 아무것도 잡지 못한다.
10. 로컬 파일 헤더의 이름·CRC·크기·method·플래그가 중앙 디렉터리와 같고, 중앙 디렉터리가 EOCD 에서 정확히 끝난다. 중앙 디렉터리만 믿으면 다른 unzip 도구가 거부하는 zip 을 통과시킬 수 있다.
11. `outDir` 가드가 동작한다. 바이너리는 바이트 그대로, 텍스트의 CRLF 는 LF 로 들어가고 그 결과가 manifest 해시와 맞는다.

9~11번은 Codex 리뷰(2026-08-29, high)에서 나온 지적을 회귀 테스트로 고정한 것이다. 각 검사는 라이터를 일부러 망가뜨려 실패하는 것을 확인했다(변이 시험 4종).
