...
> `[REQ-5.25]` SKILL.md 는 스크립트를 자신의 부모 폴더 기준 상대 경로(`scripts/scan.mjs`)로 지시한다. 경로를 얻을 수 없으면 전역 경로를 임의 탐색하지 않는다. 설치 경로를 알려 달라는 안내를 1회 남기되 **응답을 기다리지 않고** `coverage.runtime = "degraded"` 로 강등 실행한다. 이 실행이 대화형인지 판별해서는 안 된다(REQ-5.3).
