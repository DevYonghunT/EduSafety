// 심사 루브릭 — 항목·문구는 기획서 6장 표에서 출발, hackathon-2에서 트랙을 적용 조건으로 개편.
...
// when: null(모든 앱에 항상 적용) | 기능 플래그명(꺼져 있으면 자동 '해당없음' — 숨기지 않고 접어서 표시)
// 개편 이유: 트랙 근사치의 커버리지 구멍 제거 — 실데이터 항목이 교무·행정 밖에서도,
// AI 투명성 항목이 학습 콘텐츠 밖에서도 적용되어야 한다. 기능이 위험을 결정한다.
export const RUBRIC_VERSION = 'hackathon-2'

export const FEATURES = {
  studentFacing: { label: '학생이 직접 사용하는 화면이 있다', short: '학생 대면', gates: '학생 대면 보호 9항목' },
  collectsPersonalInfo: { label: '학생·학부모 개인정보를 수집한다', short: '개인정보 수집', gates: '보호 수준 L1' },
  collectsSensitiveInfo: { label: '민감정보(건강·상담·성적 상세)를 다룬다', short: '민감정보', gates: '보호 수준 L2' },
  hasAssessmentOrCompetition: { label: '평가·점수·랭킹·경쟁 기능이 있다', short: '평가·경쟁', gates: '보호 수준 L2' },
  handlesRealData: { label: '학생 실데이터(명단·성적 파일)를 다룬다', short: '실데이터 취급', gates: '실데이터 보호 2항목' },
  showsAiOutput: { label: 'AI가 생성한 내용을 화면에 보여준다', short: 'AI 출력', gates: 'AI 투명성 2항목' },
  isLearningContent: { label: '교과 학습을 목적으로 하는 콘텐츠다', short: '학습 콘텐츠', gates: '교육 적절성 3항목' },
}
...
export const rubricItems = [
  ...
  { id: 'R-under14', when: 'studentFacing', type: 'required', weight: 3, aiVerifiable: true,
    category: 'collect', authority: 'law',
  ...
  { id: 'R-admin-ext', when: 'handlesRealData', type: 'required', weight: 3, aiVerifiable: true,
  ...
  { id: 'R-score-forge', when: null, type: 'required', weight: 3, aiVerifiable: true,
  ...
  { id: 'S-minimal', when: null, type: 'scored', weight: 3, aiVerifiable: true,
    category: 'collect', authority: 'law',
    question: '최소한의 개인정보만 수집 (닉네임 대체 가능한 것은 대체)',
    plain: '덜 모을수록 안전합니다 — 닉네임으로 충분한 앱이 실명을 요구하면 감점입니다.' },
  { id: 'S-consent', when: 'studentFacing', type: 'scored', weight: 2, aiVerifiable: true,
    category: 'collect', authority: 'law',
    question: '수집 전 항목·목적·보관 기간 고지',
    plain: '무엇을, 왜, 언제까지 보관하는지 미리 알리는지 봅니다.' },
  { id: 'S-sensitive', when: 'studentFacing', type: 'scored', weight: 3, aiVerifiable: true,
    category: 'collect', authority: 'law',
    question: '민감정보(감정·건강·상담)를 익명·가명 처리 또는 기기 내 보관',
    plain: '마음 기록은 성적보다 민감합니다 — 누구 것인지 모르게 다루거나 기기 밖으로 내보내지 않아야 해요.' },
  { id: 'S-overseas', when: null, type: 'scored', weight: 1, aiVerifiable: true,
    category: 'thirdparty', authority: 'law',
  ...
  { id: 'S-xss', when: null, type: 'scored', weight: 3, aiVerifiable: true,
  ...
  { id: 'S-quota', when: null, type: 'scored', weight: 2, aiVerifiable: true,
  ...
  { id: 'S-notice', when: null, type: 'scored', weight: 1, aiVerifiable: true,
    category: 'notice', authority: 'practice',
    question: '운영자·문의처 안내',
    plain: '문제가 생겼을 때 누구에게 말하면 되는지가 앱에 적혀 있어야 합니다.' },
  { id: 'S-answer-exposure', when: 'studentFacing', type: 'scored', weight: 3, aiVerifiable: true,
  ...
  { id: 'H-retention', when: null, type: 'scored', weight: 2, aiVerifiable: false,
    category: 'notice', authority: 'law',
  ...
  { id: 'H-delete', when: null, type: 'scored', weight: 3, aiVerifiable: false,
    category: 'notice', authority: 'law',
