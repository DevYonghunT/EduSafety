...
④ 보고서를 생성하면 종합 판정 바로 아래에 심사 상태마크가 표시됩니다. `인쇄 / PDF 저장` 버튼은 서버 요청 없이 바로 인쇄 창을 열며 화면에 표시된 상태마크를 그대로 출력합니다. 상태마크에는 현재 판정, 조치 건수, 대상 저장소와 고정 commit 또는 폴더 콘텐츠 지문이 표시됩니다. 이 표시는 보고서의 판정 요약이며 별도로 발급되거나 검증되는 증명서가 아닙니다. 따라서 인증 API의 활성 정책 유무가 보고서 표시와 출력에 영향을 주지 않습니다.
