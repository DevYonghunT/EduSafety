...
최종 보고서 출력본에는 현재 판정과 고정 commit·콘텐츠 지문을 요약한 **심사 상태마크**가 포함됩니다.
...
최종 심사 보고서의 `심사 상태마크 포함 인쇄 / PDF 저장` 버튼은 서버 요청 없이 바로 인쇄 창을 엽니다. 출력 전용 상태마크에는 보고서의 현재 판정, 조치 건수, 대상 저장소와 고정 commit 또는 폴더 콘텐츠 지문이 표시됩니다. 이 표시는 보고서의 판정 요약이며 별도로 발급되거나 검증되는 증명서가 아닙니다. 따라서 인증 API의 활성 정책 유무가 보고서 출력에 영향을 주지 않습니다.
